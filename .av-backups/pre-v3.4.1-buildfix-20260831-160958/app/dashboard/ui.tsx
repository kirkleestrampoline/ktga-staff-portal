  ...
  function LeaveView(){
    ...
    if(isAdmin){
      ...
      return <><PageHead title="Leave Management" sub="Review and manage staff leave and unavailable periods."><select value={scheduleFilter} onChange={e=>setScheduleFilter(e.target.value)}><option value="">All organisations</option>{adminVenues().map(v=><option key={v.id} value={v.id}>{v.name}</option>)}</select></PageHead>
        ...
        <section className="card section v341LeaveCalendar"><div className="sectionHeader"><div><h3>Monthly leave calendar</h3><p>{monthLabel(month)} · approved and pending time away.</p></div><MonthSelect/></div><div className="v341CalendarGrid">{monthDates.map(date=>{const items=leaveScope.filter(r=>date>=r.start_date&&date<=r.end_date&&r.status!=="cancelled"&&r.status!=="declined");return <div className={`v341CalendarDay ${items.length?"hasItems":""}`} key={date}><strong>{new Date(`${date}T12:00:00`).getDate()}</strong><span>{new Date(`${date}T12:00:00`).toLocaleDateString("en-GB",{weekday:"short"})}</span>{items.slice(0,3).map(r=><button key={r.id} className={`v341CalendarPill ${r.status} v331Type-${r.request_type}`} onClick={()=>openEditTimeAway(r)}><b>{profileById(r.profile_id)?.full_name?.split(" ")[0]||"Staff"}</b><small>{r.request_type==="unavailable"?"Unavailable":"Leave"}{!r.all_day?` ${r.start_time?.slice(0,5)}–${r.end_time?.slice(0,5)}`:""}</small></button>)}{items.length>3&&<em>+{items.length-3} more</em>}</div>})}</div></section>
