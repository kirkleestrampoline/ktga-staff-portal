-- Forward only, after v1_7_0. PREPARED ONLY: review and apply manually.
-- Stores payment preferences only; existing Load shifts/payment behaviour unchanged.
begin;
set local lock_timeout='2s';
set local statement_timeout='60s';
do $preflight$
begin
 if to_regprocedure('public.classes_save_profile(jsonb,jsonb)') is null then raise exception 'Apply and verify Classes Phase 1 first'; end if;
 if (select md5(prosrc) from pg_proc where oid='public.classes_save_profile(jsonb,jsonb)'::regprocedure)<>'7db239893e09f5b52496eb54121c3a74' then raise exception 'classes_save_profile differs from reviewed Phase 1'; end if;
 if (select md5(prosrc) from pg_proc where oid='public.classes_calendar_data(date,date)'::regprocedure)<>'e23657248ab5b0f3d84e4a235d17da28' then raise exception 'classes_calendar_data differs from reviewed Phase 1'; end if;
 if exists(select 1 from information_schema.columns where table_schema='public' and table_name='class_staffing_slots' and column_name='default_payment_type') then raise exception 'Creation upgrade already installed or schema drift'; end if;
end $preflight$;
alter table public.class_staffing_slots add column default_payment_type text not null default 'standard' check(default_payment_type in ('standard','enhanced','volunteer'));
