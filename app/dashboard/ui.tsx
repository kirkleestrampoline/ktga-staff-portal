...
export default function Dashboard({initialProfile,initialTab,initialMonth}:{initialProfile:Profile;initialTab:Tab;initialMonth:string}){
  ...
  async function sendPasswordResetEmail(s:Profile){
    if(!s.email&&!s.contact_email){setSecurityActionMessage("Add a recovery email before sending a password reset.");return}
    setSecurityActionBusy(true);setSecurityActionMessage("Sending password reset email…");
    try{
  ...
  function ScheduleView(){
    ...
    return <div className="v513ScheduleWorkspace"><PageHead centered title="Schedule & Staffing" sub="Build once, copy forward and only change the exceptions."><MonthNavigation/></PageHead>
      ...
      <div className="v513MobileActions"><div className="v513PrimaryActions"><button className="btn btnAccent" type="button" onClick={()=>openNewClass()}><PlusIcon/>Add Class</button><button className="btn btnSecondary" type="button" onClick={()=>{setMasterTimetableDay(null);setMasterTimetableOpen(true)}}>Open Master Timetable</button><button className="btn btnPrimary" type="button" onClick={generateSchedule}>Load Shifts</button></div><div className="v513SecondaryActions"><div className="v313MoreWrap"><button className="btn btnSecondary" type="button" onClick={()=>setMonthActionsOpen(!monthActionsOpen)}>More <span className="v313Chevron">⌄</span></button>{monthActionsOpen&&<><button className="v313MenuScrim" aria-label="Close month actions" onClick={()=>setMonthActionsOpen(false)}/><div className="v313MoreMenu"><button type="button" onClick={()=>{setMonthActionsOpen(false);void clonePreviousScheduleMonth()}}>Duplicate previous month</button><button type="button" onClick={()=>{setMonthActionsOpen(false);void copyScheduleWeek()}}>Copy a week</button><div className="v313MenuDivider"/><button type="button" className="danger" onClick={()=>{setMonthActionsOpen(false);void clearScheduleMonth()}}>Clear this month</button></div></>}</div></div></div>
      <section className="card v515Configuration"><div><span>Configuration</span><p>Manage recurring classes and monthly schedule setup.</p></div><div className="v515ConfigurationActions"><button className="btn btnSecondary" type="button" onClick={()=>{setMasterTimetableDay(null);setMasterTimetableOpen(true)}}>Open Master Timetable</button><button className="btn btnPrimary" type="button" onClick={generateSchedule}>Load Shifts</button><button className="btn btnSecondary" type="button" onClick={clonePreviousScheduleMonth}>Duplicate Previous Month</button><div className="v313MoreWrap"><button className="btn btnSecondary" type="button" onClick={()=>setMonthActionsOpen(!monthActionsOpen)}>More <span className="v313Chevron">⌄</span></button>{monthActionsOpen&&<><button className="v313MenuScrim" aria-label="Close month actions" onClick={()=>setMonthActionsOpen(false)}/><div className="v313MoreMenu"><button type="button" onClick={()=>{setMonthActionsOpen(false);void copyScheduleWeek()}}>Copy a week</button><div className="v313MenuDivider"/><button type="button" className="danger" onClick={()=>{setMonthActionsOpen(false);void clearScheduleMonth()}}>Clear this month</button></div></>}</div></div></section>
