  ...
  function AdminScheduleShiftModal(){
    ...
    return <div className="modalBackdrop"><div className="modal v311AdminShiftModal v405ScheduleControlModal">
      ...
      <div className="modalBody v405ScheduleControlBody">
        ...
        <div className="field"><label>Assigned coach</label><div className="v400AssignmentPanel">{(["available","pending","away","working"] as const).map(group=>{const people=allowed.map(p=>({p,state:coachAssignmentState(p.id,s)})).filter(x=>x.state.state===group);if(!people.length)return null;return <div className={`v400AssignGroup ${group}`} key={group}><span>{group==="available"?"Available":group==="pending"?"Pending request":group==="away"?"Away":"Already coaching"}</span><div>{people.map(({p,state})=>{const conflict=group==="working"?scheduledOverlapsForCoach(p.id,s)[0]:null;return <button type="button" disabled={s.status==="cancelled"||s.status==="confirmed"} className={s.profile_id===p.id?"selected":""} key={p.id} onClick={()=>assignCoach(p.id)}><strong>{p.full_name}</strong>{conflict?<span className="v405CoachingConflict"><b>{conflict.class_name}</b><small>{conflict.start_time.slice(0,5)}–{conflict.finish_time.slice(0,5)} · {venueName(conflict.venue_id)}</small></span>:<small>{state.label}</small>}</button>})}</div></div>})}<button type="button" className="v400Unassign" disabled={!s.profile_id||s.status==="cancelled"||s.status==="confirmed"} onClick={async()=>{await reassignScheduledWithAvailability(s,"");setAdminScheduleShift({...s,profile_id:null})}}>Set unassigned</button></div></div>
