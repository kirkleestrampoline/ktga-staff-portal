...
import ActualTimeCard from "@/components/actual-time-card";
import { validateActualTimes, confirmActualBatch, approvalTimeRequest, canEditShift, type ActualTimes } from "@/lib/timesheet-actual";
...
import { fetchClubOverviewSummary, type ClubOverviewSummary } from "@/lib/dashboard/overview";
import { groupCapacityRows, plural, programmeState, sortCapacityGroups, type CapacityGroup } from "@/lib/dashboard/club-overview-ui";
import { staffingCostSnapshot } from "@/lib/dashboard/staffing-costs";
...
export default function Dashboard({initialProfile,initialTab,initialMonth,launchResetEnabled=false}:{initialProfile:Profile;initialTab:Tab;initialMonth:string;launchResetEnabled?:boolean}){
  ...
  const canManageRoles=isPlatformAdmin||isClubOwner;
  const [tab,setTabState]=useState<Tab>(initialTab);
  const [overviewView,setOverviewView]=useState<'today'|'dashboard'>('today');
  const [dashboardPanel,setDashboardPanel]=useState<'overview'|'members'|'classes'|'staffing'|'finance'>('overview');
  const [capacitySort,setCapacitySort]=useState<'priority'|'name'|'spaces'>('priority');
  const [capacityDetail,setCapacityDetail]=useState<CapacityGroup|null>(null);
  const capacityDetailReturnFocus=useRef<HTMLButtonElement|null>(null);
  const [overviewHydrated,setOverviewHydrated]=useState(false);
  const [overviewSummary,setOverviewSummary]=useState<ClubOverviewSummary|null>(null),[overviewSummaryState,setOverviewSummaryState]=useState<"loading"|"loaded"|"unavailable"|"error">("loading");
  ...
  const [shiftAccess,setShiftAccess]=useState<{key:string;editable:boolean}|null>(null);
  const [shiftModal,setShiftModal]=useState<Shift|null>(null);
  const [inviteOpen,setInviteOpen]=useState(false);
  const [staffEdit,setStaffEdit]=useState<Profile|null>(null);
  const [invite,setInvite]=useState({name:"",username:"",password:"",email:"",rate:"",enhancedRate:"",employmentType:"hourly" as "hourly"|"salaried",canVolunteer:false,annualSalary:"",contractedWeeklyHours:"",workingWeeksPerYear:"",portalAccess:true});
  ...
  const [adminCostStatus,setAdminCostStatus]=useState<"loading"|"ready"|"error">("loading");
  ...
  const [classesSetupRequested,setClassesSetupRequested]=useState(false);
  const [classesReturnLibrary,setClassesReturnLibrary]=useState(false);
  useEffect(()=>{if(!classesSetupRequested)return;const onPop=()=>{setClassesSetupRequested(false);setClassesReturnLibrary(true);setTab("classes")};window.addEventListener("popstate",onPop);return()=>window.removeEventListener("popstate",onPop)},[classesSetupRequested]);
  ...
  const paymentAssignmentReturnFocus=useRef<HTMLButtonElement|null>(null);
  ...
  const overviewSummaryRequest=useRef(0);
  ...
  useEffect(()=>{
    if(!isAdmin){setOverviewHydrated(true);return}
    const params=new URLSearchParams(window.location.search);
    const requested=params.get('view')==='dashboard'?'dashboard':'today';
    const requestedPanel=params.get('panel');
    setOverviewView(requested);
    if(requestedPanel==='members'||requestedPanel==='classes'||requestedPanel==='staffing'||requestedPanel==='finance'||requestedPanel==='overview')setDashboardPanel(requestedPanel);
  ...
  const shiftAccessKey=shiftModal?`${shiftModal.id||"new"}:${shiftModal.coach_id}:${shiftModal.shift_date}`:"";
  const shiftEditable=shiftAccess?.key===shiftAccessKey&&shiftAccess.editable;
  useEffect(()=>{
    if(!shiftModal){setShiftAccess(null);return}
    setShiftAccess(null);
    let cancelled=false;
    const key=shiftAccessKey;
    void (async()=>{
      const [{data,error},target]=await Promise.all([
        supabase.from("timesheets").select("status").eq("coach_id",shiftModal.coach_id).eq("month_start",`${shiftModal.shift_date.slice(0,7)}-01`).maybeSingle(),
        supabase.from("profiles").select("club_id").eq("id",shiftModal.coach_id).single()
      ]);
      if(!cancelled)setShiftAccess({key,editable:!error&&!target.error&&Boolean(initialProfile.club_id)&&target.data?.club_id===initialProfile.club_id&&canEditShift(isAdmin,Boolean(shiftModal.id),data?.status)});
    })();
  ...
  function setOverviewSection(view:'today'|'dashboard'){
    const url=new URL(window.location.href);url.search="";url.searchParams.set('tab','dashboard');url.searchParams.set('view',view);if(view==='dashboard')url.searchParams.set('panel',dashboardPanel);window.history.pushState({},'',`${url.pathname}${url.search}${url.hash}`);setOverviewView(view);setTabState('dashboard');requestAnimationFrame(()=>{overviewContentRef.current?.scrollTo({top:0});overviewHeadingRef.current?.focus({preventScroll:true})});
  }
  function setDashboardOverviewPanel(panel:'overview'|'members'|'classes'|'staffing'|'finance'){
    const url=new URL(window.location.href);url.search="";url.searchParams.set('tab','dashboard');url.searchParams.set('view','dashboard');url.searchParams.set('panel',panel);window.history.pushState({},'',`${url.pathname}${url.search}${url.hash}`);setDashboardPanel(panel);setOverviewView('dashboard');
  }
  function openClassLibrary(){
    setClassesReturnLibrary(true);
    const url=new URL(window.location.href);url.search='';url.searchParams.set('tab','classes');window.history.pushState({},'',`${url.pathname}${url.search}${url.hash}`);setTabState('classes');
  }
  ...
  useEffect(()=>{
    ...
    if(canonical==='dashboard') { const view=initialUrl.searchParams.get('view'); initialUrl.search=""; initialUrl.searchParams.set('tab','dashboard'); initialUrl.searchParams.set('view',view==='dashboard'?'dashboard':'today'); window.history.replaceState({},'',`${initialUrl.pathname}${initialUrl.search}${initialUrl.hash}`); }
    ...
    const initialView=initialUrl.searchParams.get('view');if(initialView==='today'||initialView==='dashboard')setOverviewView(initialView);
    const handleHistoryNavigation=()=>{
      const next=dashboardTabForRole(new URL(window.location.href).searchParams.get("tab"),initialProfile.role);
      const currentUrl=new URL(window.location.href);
      if(next==='dashboard') { const view=currentUrl.searchParams.get('view'); currentUrl.search=""; currentUrl.searchParams.set('tab','dashboard'); currentUrl.searchParams.set('view',view==='dashboard'?'dashboard':'today'); }
      ...
      setTabState(next);
      const historyView=currentUrl.searchParams.get('view');if(historyView==='today'||historyView==='dashboard')setOverviewView(historyView);else if(next==='dashboard')setOverviewView('today');
      const historyPanel=currentUrl.searchParams.get('panel');if(historyPanel==='overview'||historyPanel==='members'||historyPanel==='classes'||historyPanel==='staffing'||historyPanel==='finance')setDashboardPanel(historyPanel);else setDashboardPanel('overview');
    };
    window.addEventListener("popstate",handleHistoryNavigation);
    return()=>window.removeEventListener("popstate",handleHistoryNavigation);
  },[initialProfile,initialMonth]);
  ...
  useEffect(()=>{
    ...
    const bodyOverflow=document.body.style.overflow;
    const close=(event:KeyboardEvent)=>{if(document.querySelector("dialog[open]"))return;if(event.key==="Escape"){event.preventDefault();if(paymentAssignment){closePaymentAssignment();return}setAdminScheduleShift(null);setStaffingRecommendationShift(null);setStaffingQualificationContext(null)}};
    document.body.style.overflow="hidden";
    window.addEventListener("keydown",close);
    return()=>{document.body.style.overflow=bodyOverflow;window.removeEventListener("keydown",close)};
  },[adminScheduleShift,staffingRecommendationShift,paymentAssignment]);
  ...
  async function loadOverviewSummary(){
    if(!isAdmin){setOverviewSummaryState("unavailable");return}
    const requestId=++overviewSummaryRequest.current;
    setOverviewSummaryState("loading");
    try {
      const summary=await fetchClubOverviewSummary(supabase,localDateKey());
      if(overviewSummaryRequest.current!==requestId)return;
      setOverviewSummary(summary);setOverviewSummaryState("loaded");
    } catch(error:any) {
      if(overviewSummaryRequest.current!==requestId)return;
      setOverviewSummaryState(error?.code==='42883'||error?.code==='PGRST202'?"unavailable":"error");
    }
  }
  ...
  async function loadTabData(next:Tab){
    if(next==="availability"&&isAdmin)await Promise.all([runSharedDataLoad("venues",loadVenues),runSharedDataLoad("staff",loadStaff),runSharedDataLoad("leave",loadLeaveData),runSharedDataLoad("future-schedule",loadFutureUnstaffedShifts),runSharedDataLoad(`overview-schedule:${month}`,loadOverviewSchedule)]);
      ...
      if(isAdmin)await Promise.all([loadSchedule(),runSharedDataLoad("staff",loadStaff),runSharedDataLoad(`extra-shifts:${month}`,loadPendingExtraShifts),loadAdmin(true)]);
    ...
    else if(next==="workforce"&&isAdmin)await Promise.all([runSharedDataLoad("venues",loadVenues),runSharedDataLoad("staff",loadStaff),loadAdmin(false),loadExpenses()]);
  ...
  async function reloadLoadedTab(current:Tab){
    if(current==="schedule")await Promise.all([loadSchedule(),isAdmin?runSharedDataLoad("staff",loadStaff):Promise.resolve(),isAdmin?loadPendingExtraShifts():Promise.resolve(),isAdmin?loadAdmin(true):Promise.resolve()]);
    ...
    else if(current==="workforce"&&isAdmin)await Promise.all([loadAdmin(false),loadExpenses()]);
  ...
  async function loadVenues(){
    ...
    setManagedVenueIds(isAdmin?((data||[]) as Venue[]).map(v=>v.id):(links||[]).filter((x:any)=>x.profile_id===initialProfile.id&&x.is_admin).map((x:any)=>x.venue_id));
  }
  ...
  function clubVenue(){return venues[0]}
  ...
  async function loadStaffingIntelligenceSettings(){
    const{data,error}=await supabase.from("staffing_recommendation_settings").select("mandatory_rules,criteria,priority_order").eq("club_id",initialProfile.club_id||"").maybeSingle();
    if(error){
  ...
  async function saveStaffingIntelligenceSettings(){
    ...
    const{error}=await supabase.from("staffing_recommendation_settings").update({mandatory_rules:staffingIntelligence.mandatory_rules,criteria:staffingIntelligence.criteria,priority_order:staffingIntelligence.priority_order,priorities,updated_at:new Date().toISOString(),updated_by:initialProfile.id}).eq("club_id",initialProfile.club_id||"");
    setSaving(false);
  ...
  async function loadAdmin(includeInvoices=true){
    ...
    if(coachesError){setAdminCostStatus("error");throw coachesError}
    if(shiftsError){setAdminCostStatus("error");throw shiftsError}
    if(timesheetsError){setAdminCostStatus("error");throw timesheetsError}
    ...
    if(includeInvoices&&tids.length){
      ...
      if(error){setAdminCostStatus("error");throw error}
      inv=(data||[]) as Invoice[];
    }
    if(requestedMonth!==currentMonthRef.current||requestId!==adminRequestSequence.current)return;
    setAdminMonthShifts((ss||[]) as Shift[]);
    setAdminCostStatus("ready");
  ...
  async function saveEmploymentRecord(){
    ...
    if(d.employment_type==="salaried"&&(d.annual_salary===null||!Number.isFinite(Number(d.annual_salary))||Number(d.annual_salary)<0||d.contracted_weekly_hours===null||!Number.isFinite(Number(d.contracted_weekly_hours))||Number(d.contracted_weekly_hours)<=0||Number(d.contracted_weekly_hours)>168||d.working_weeks_per_year===null||!Number.isFinite(Number(d.working_weeks_per_year))||Number(d.working_weeks_per_year)<1||Number(d.working_weeks_per_year)>52)){flash("Salaried records require a non-negative salary, contracted hours up to 168, and working weeks between 1 and 52.");return}
  ...
  async function saveBusiness(){
    ...
    }).eq("club_id",initialProfile.club_id||"");
    setSaving(false);
  ...
  async function sendInvite(e:FormEvent){
    ...
    const inviteAnnualSalary=Number(invite.annualSalary),inviteWeeklyHours=Number(invite.contractedWeeklyHours),inviteWorkingWeeks=Number(invite.workingWeeksPerYear);
    if(invite.employmentType==="salaried"&&(invite.annualSalary===""||!Number.isFinite(inviteAnnualSalary)||inviteAnnualSalary<0||invite.contractedWeeklyHours===""||!Number.isFinite(inviteWeeklyHours)||inviteWeeklyHours<=0||inviteWeeklyHours>168||invite.workingWeeksPerYear===""||!Number.isFinite(inviteWorkingWeeks)||inviteWorkingWeeks<1||inviteWorkingWeeks>52)){flash("Salaried staff require a non-negative salary, contracted hours up to 168, and working weeks between 1 and 52.");return}
    setSaving(true);
    const res=await fetch("/api/staff-access",{
      ...
      body:JSON.stringify({
        ...
        employment_type:invite.employmentType,
        standard_rate:Number(invite.rate),
        enhanced_rate:Number(invite.enhancedRate||invite.rate),
        can_volunteer:invite.canVolunteer,
        annual_salary:invite.employmentType==="salaried"?Number(invite.annualSalary):null,
        contracted_weekly_hours:invite.employmentType==="salaried"?Number(invite.contractedWeeklyHours):null,
        working_weeks_per_year:invite.employmentType==="salaried"?Number(invite.workingWeeksPerYear):null,
    ...
    setInviteOpen(false);
    setInvite({name:"",username:"",password:"",email:"",rate:"",enhancedRate:"",employmentType:"hourly",canVolunteer:false,annualSalary:"",contractedWeeklyHours:"",workingWeeksPerYear:"",portalAccess:true});
    setInviteVenueIds([]);setInviteRole("coach");
    void loadStaff();void loadAdmin();
  }

  async function saveShift(){
    if(!shiftModal||!shiftEditable)return;
    if(shiftModal.scheduled_shift_id){
      const {error}=await supabase.rpc("confirm_scheduled_actual",{p_scheduled_id:shiftModal.scheduled_shift_id,p_start_time:shiftModal.start_time,p_finish_time:shiftModal.finish_time,p_break_minutes:shiftModal.break_minutes});
      if(error){flash(error.message);return}
      setShiftModal(null);await loadCoachMonth(shiftModal.coach_id);await loadSchedule();await loadAdmin();return;
    }
  ...
  async function deleteShift(){
    if(!shiftModal?.id||!shiftEditable||shiftModal.scheduled_shift_id)return;
  ...
  async function confirmDailySelection(){
    if(!dailyConfirmation||dailyFlight.current)return;
    const review=dailyConfirmation;
    const selected=eligibleDailyConfirmations(review.date,review.profileId).filter(shift=>review.selectedIds.includes(shift.id)&&!review.succeeded?.includes(shift.id));
    if(!selected.length)return;
    dailyFlight.current=true;setSaving(true);
    try{
  ...
  async function openAdjustment(sch:ScheduledShift){
    const {data:ts,error}=await supabase.from("timesheets").select("status").eq("coach_id",sch.profile_id).eq("month_start",`${sch.shift_date.slice(0,7)}-01`).maybeSingle();
    if(error||ts?.status==="submitted"||ts?.status==="paid"){flash(error?.message||"That month is locked.");return}
    let actual:Shift|null=null;
    if(sch.actual_shift_id){
      const result=await supabase.from("shifts").select("*").eq("id",sch.actual_shift_id).single();
      if(result.error){flash(result.error.message);return}
      actual=result.data;
    }
    setAdjustShift(sch);
    setAdjustStart((actual?.start_time||sch.requested_start_time||sch.start_time).slice(0,5));
    setAdjustFinish((actual?.finish_time||sch.requested_finish_time||sch.finish_time).slice(0,5));
    setAdjustBreak(Number(actual?.break_minutes??sch.requested_break_minutes??sch.break_minutes??0));
  ...
  async function submitRotaActual(){
    ...
    const fn=isAdmin||actual<=scheduled?"confirm_scheduled_actual":"request_scheduled_overtime";
    const args=fn==="confirm_scheduled_actual"
  ...
  async function approveExtraShift(s:Shift){
    if(!s.id)return;
    const request=approvalTimeRequest(s.id,{start:s.start_time,finish:s.finish_time,breakMinutes:Number(s.break_minutes||0)});
  ...
  const actualCostDisplay=adminCostStatus==="ready"?money(actualScheduleCost):adminCostStatus==="error"?"Unavailable":"Loading…";
  const actualCostFoot=adminCostStatus==="ready"?"Confirmed timesheet hours":adminCostStatus==="error"?"Could not load confirmed hours":"Loading confirmed timesheet hours";
  ...
  const staffingCosts=staffingCostSnapshot({normalStaffingCost:normalCost,currentForecast:forecastCost,actualCostSoFar:actualScheduleCost});
  ...
  return <div className="portal">
    ...
    <div className="mainWrap">
      <header className="topbar"><div className="row v3TopbarIdentity"><ClubIdentity name={currentClub?.name||"Club workspace"} logoUrl={currentClub?.logo_url} primaryColour={currentClub?.primary_colour}/></div><div className="topActions"><span className="versionBadge">v5.1</span><span className="muted desktopEmail" style={{fontSize:12}}>{initialProfile.email}</span></div></header>
      <main className="main">
        ...
        {tab!=="dashboard"&&tabLoadError?.tab===tab?TabLoadFailure():tab!=="dashboard"&&(loadingTab===tab||!loadedTabs.has(tab))?TabLoadingSkeleton():<>
          ...
          {tab==="classes"&&<ClassesView initialProfileId={classesRequestedId} initialSection={classesReturnLibrary?'Class Library':undefined} onOpened={()=>{setClassesRequestedId(null);setClassesReturnLibrary(false)}} onOpenClassSetup={()=>{setClassesSetupRequested(true);window.history.pushState({},"",`${window.location.pathname}?tab=settings&section=class-setup`);setTab("settings")}} canEdit={isAdmin} registersOnly={!isAdmin}/>}
    ...
    <MobileNav tab={tab} setTab={(t:Tab)=>{setAdminPersonalRota(false);setTab(t);if(t!=="timesheets")backToAdmin()}} role={initialProfile.role} name={initialProfile.full_name} clubName={currentClub?.name} logoUrl={currentClub?.logo_url} primaryColour={currentClub?.primary_colour} open={mobileMoreOpen} setOpen={setMobileMoreOpen} onSignOut={signOut}/>
  </div>;
  ...
  function LegacyTodayCommandView(){
    const today=localDateKey();
    const classesToday=scheduledShifts.filter(s=>s.shift_date===today&&s.status!=="cancelled").sort((a,b)=>a.start_time.localeCompare(b.start_time));
    const unassigned=classesToday.filter(s=>!s.profile_id).length;
    return <div className="clubOverviewToday"><PageHead centered dashboard title={`Good ${new Date().getHours()<12?"morning":new Date().getHours()<18?"afternoon":"evening"}, ${initialProfile.full_name.split(" ")[0]}`} sub="Here’s what’s happening across your club today."><div className="clubOverviewViewSwitch" role="tablist" aria-label="Club Overview view"><button type="button" role="tab" aria-selected={overviewView==='today'} onClick={()=>setOverviewSection('today')}>Today</button><button type="button" role="tab" aria-selected={overviewView==='dashboard'} onClick={()=>setOverviewSection('dashboard')}>Dashboard</button></div></PageHead><p className="clubOverviewDate">{new Date(`${today}T12:00:00`).toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</p><section className="clubOverviewSummaryGrid" aria-label="Today at a glance"><button className="clubOverviewSummaryCard" type="button" onClick={()=>setTab('schedule')}><strong>{classesToday.length}</strong><span>Classes today</span></button><div className="clubOverviewSummaryCard"><strong>Unavailable</strong><span>Active athletes expected today</span><small>Not available from existing reads.</small></div><div className="clubOverviewSummaryCard"><strong>Unavailable</strong><span>Trials attending today</span><small>Not available from existing reads.</small></div></section><section className="clubOverviewAttention" aria-labelledby="today-attention"><div><span className="clubOverviewEyebrow">PRIORITY</span><h2 id="today-attention">Needs attention</h2></div>{unassigned>0?<button className="clubOverviewAttentionItem" type="button" onClick={()=>setTab('schedule')}><strong>{unassigned} staffing issue{unassigned===1?'':'s'}: unassigned shift{unassigned===1?'':'s'}</strong><span>Open Schedule &amp; Staffing</span></button>:<div className="clubOverviewAllClear"><strong>All clear for today</strong><span>No supported staffing issues need attention.</span></div>}</section><section className="clubOverviewProgramme" aria-labelledby="today-programme"><div className="sectionHeader"><div><span className="clubOverviewEyebrow">TODAY</span><h2 id="today-programme">Today’s programme</h2></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('schedule')}>View staffing</button></div>{classesToday.length? <div className="clubOverviewTimeline">{classesToday.map(s=><article key={s.id} className="clubOverviewTimelineRow"><time>{s.start_time.slice(0,5)}<small>{s.finish_time.slice(0,5)}</small></time><div><strong>{s.class_name}</strong><span>{venueName(s.venue_id)}</span><small>Expected athletes: unavailable · Available places: unavailable</small></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>View class</button></article>)}</div>:<div className="clubOverviewAllClear"><strong>No classes scheduled today</strong><span>Published staffing data is currently available.</span></div>}</section><section className="clubOverviewQuickActions" aria-label="Quick actions"><button type="button" className="btn btnSecondary" onClick={()=>setTab('members')}>Add member</button><button type="button" className="btn btnSecondary" onClick={()=>setTab('members')}>Enrol athlete</button><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>Create class</button><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>Open today’s registers</button><button type="button" className="btn btnSecondary" onClick={()=>setTab('schedule')}>Add shift</button></section></div>;
  }
  function ClubOverviewHero(){
    return <section className="clubOverviewHero"><div><span className="clubOverviewEyebrow">CLUB OVERVIEW</span><h1 ref={overviewHeadingRef} tabIndex={-1}>Good evening, {initialProfile.full_name.split(' ')[0]}</h1><p>Plan the day, monitor capacity and keep your club moving.</p><small>{new Date(`${localDateKey()}T12:00:00`).toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}</small></div><div className="clubOverviewViewSwitch" role="tablist" aria-label="Club Overview view"><button type="button" role="tab" aria-selected={overviewView==='today'} onClick={()=>setOverviewSection('today')}>Today</button><button type="button" role="tab" aria-selected={overviewView==='dashboard'} onClick={()=>setOverviewSection('dashboard')}>Dashboard</button></div></section>;
  }
  function LegacyTodayCommandViewV2(){
    const summary=overviewSummary;
    const todayWeekday=new Date(`${localDateKey()}T12:00:00`).getDay();
    const rows=(summary?.class_rows||[]).filter(row=>row.weekday===todayWeekday).slice().sort((a,b)=>a.start_time.localeCompare(b.start_time));
    const value=(ready:number|undefined)=>overviewSummaryState==='loaded'&&typeof ready==='number'?ready.toLocaleString('en-GB'):'Unavailable';
    const support=overviewSummaryState==='unavailable'?'Club summary is temporarily unavailable.':overviewSummaryState==='error'?'Unable to load the club summary. Try again.':overviewSummaryState==='loading'?'Loading club summary…':'';
    return <div className="clubOverviewToday"><PageHead centered dashboard title="Today" sub="Your club at a glance."><div className="clubOverviewViewSwitch" role="tablist" aria-label="Club Overview view"><button type="button" role="tab" aria-selected={overviewView==='today'} onClick={()=>setOverviewSection('today')}>Today</button><button type="button" role="tab" aria-selected={overviewView==='dashboard'} onClick={()=>setOverviewSection('dashboard')}>Dashboard</button></div></PageHead>{support&&<div className={`clubOverviewDataNotice ${overviewSummaryState==='error'?'error':''}`} role={overviewSummaryState==='error'?'alert':'status'}>{support}</div>}<section className="clubOverviewSummaryGrid" aria-label="Today at a glance"><div className="clubOverviewSummaryCard"><strong>{value(summary?.today.class_sessions_today)}</strong><span>Classes today</span></div><div className="clubOverviewSummaryCard"><strong>{value(summary?.today.unique_athletes_expected_today)}</strong><span>Athletes expected today</span></div>{summary&&<div className="clubOverviewSummaryCard"><strong>{value(summary.today.trial_athletes_expected_today)}</strong><span>Trials attending today</span></div>}</section><section className="clubOverviewProgramme" aria-labelledby="today-programme"><div className="sectionHeader"><div><span className="clubOverviewEyebrow">TODAY</span><h2 id="today-programme">Today’s programme</h2><p>{summary?`${summary.today.expected_attendances_today.toLocaleString('en-GB')} expected attendances`:''}</p></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('schedule')}>View staffing</button></div>{rows.length?<div className="clubOverviewTimeline">{rows.map(row=><article key={row.class_id} className="clubOverviewTimelineRow"><time>{row.start_time.slice(0,5)}<small>{row.finish_time.slice(0,5)}</small></time><div><strong>{row.class_name}</strong><span>{row.venue_name||'Venue unavailable'}</span><small>{row.active_places.toLocaleString('en-GB')} expected · {row.available_spaces===null?'Places unavailable':`${Math.max(0,row.available_spaces).toLocaleString('en-GB')} places available`}{row.waiting_list?` · ${row.waiting_list} waiting`:''}</small></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>View class</button></article>)}</div>:<div className="clubOverviewAllClear"><strong>{overviewSummaryState==='loading'?'Loading today’s programme…':'No classes scheduled today'}</strong><span>{overviewSummaryState==='loaded'?'Published classes will appear here.':'The programme will appear when the summary is available.'}</span></div>}</section></div>;
  }
  function TodayMetricGrid({summary,rows}:{summary:ClubOverviewSummary;rows:ClubOverviewSummary['class_rows']}){
    const now=new Date();const state=programmeState(rows,now.getHours()*60+now.getMinutes());
    const next=state.row;const label=state.kind==='running'?'Happening now':state.kind==='complete'?'Programme complete':state.kind==='empty'?'No classes today':'Next class';
    return <section className="clubOverviewMetricGrid" aria-label="Today at a glance"><article><i aria-hidden="true">▣</i><strong>{summary.today.class_sessions_today}</strong><span>Classes today</span><small>{plural(summary.today.class_sessions_today,'session')}</small></article><article><i aria-hidden="true">◉</i><strong>{summary.today.unique_athletes_expected_today}</strong><span>Athletes expected</span><small>{summary.today.unique_athletes_expected_today?'Expected today':'No athletes expected'}</small></article><article><i aria-hidden="true">◇</i><strong>{summary.today.trial_athletes_expected_today}</strong><span>Trials attending</span><small>{summary.today.trial_athletes_expected_today?plural(summary.today.trial_athletes_expected_today,'trial'):'No trials attending'}</small></article><article className="next"><i aria-hidden="true">◷</i><strong>{next?(state.kind==='complete'?next.finish_time.slice(0,5):state.kind==='running'?next.finish_time.slice(0,5):next.start_time.slice(0,5)):'—'}</strong><span>{label}</span><small>{next?.class_name||'No sessions scheduled'}</small></article></section>;
  }
  function LegacyTodayCommandViewCurrent(){
    const summary=overviewSummary;const weekday=new Date(`${localDateKey()}T12:00:00`).getDay();const rows=(summary?.class_rows||[]).filter(row=>row.weekday===weekday).sort((a,b)=>a.start_time.localeCompare(b.start_time));
    if(!summary)return <div className="clubOverviewToday"><div className="clubOverviewCompatibility" role="status">{overviewSummaryState==='loading'?'Loading today’s programme…':'Today’s club summary is not available yet.'}</div></div>;
    const unassigned=scheduledShifts.filter(shift=>shift.shift_date===localDateKey()&&!shift.profile_id&&shift.status!=='cancelled').length;
    return <div className="clubOverviewToday"><TodayMetricGrid summary={summary} rows={rows}/><div className="clubOverviewTodayLayout"><section className="clubOverviewProgramme clubOverviewTimelineCard" aria-labelledby="today-programme"><div className="clubOverviewSectionHead"><div><span className="clubOverviewEyebrow">TODAY’S PROGRAMME</span><h2 id="today-programme">Sessions in time order</h2></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>Open timetable</button></div>{rows.length?<div className="clubOverviewTimeline">{rows.map(row=>{const used=row.capacity?Math.min(100,Math.max(0,row.active_places/row.capacity*100)):0;const state=row.available_spaces!==null&&row.available_spaces<0?'over':row.available_spaces===0?'full':row.capacity&&row.available_spaces!==null&&row.available_spaces/row.capacity<=.2?'near':row.active_places===0?'empty':'healthy';return <article key={row.class_id} className={`clubOverviewTimelineRow ${state}`}><time>{row.start_time.slice(0,5)}<small>{row.finish_time.slice(0,5)}</small></time><div><strong>{row.class_name}</strong><span>{row.venue_name}</span><small>{plural(row.active_places,'athlete')} · {row.capacity===null?'Capacity unavailable':`${row.active_places}/${row.capacity} · ${row.available_spaces} ${row.available_spaces===1?'space':'spaces'}`}{row.waiting_list?` · ${plural(row.waiting_list,'waiting')}`:''}</small>{row.capacity!==null&&<span className="clubOverviewUtilisation" role="progressbar" aria-label={`${row.class_name} utilisation`} aria-valuenow={Math.round(used)} aria-valuemin={0} aria-valuemax={100}><b style={{width:`${used}%`}}/></span>}</div><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>View class</button></article>})}</div>:<div className="clubOverviewAllClear"><strong>No classes today</strong><span>There are no published sessions for today.</span></div>}</section><aside className="clubOverviewTodayRail"><section className="clubOverviewRailCard"><span className="clubOverviewEyebrow">DAY SUMMARY</span><strong>{rows.length?`${rows[0].start_time.slice(0,5)} – ${rows[rows.length-1].finish_time.slice(0,5)}`:'No sessions'}</strong><small>{plural(rows.length,'session')} · {plural(summary.today.expected_attendances_today,'expected attendance')}</small></section><section className="clubOverviewRailCard"><span className="clubOverviewEyebrow">NEEDS ATTENTION</span><strong>{unassigned?plural(unassigned,'unassigned staffing item'):'Everything is ready for today'}</strong><small>{unassigned?'Open staffing to assign support.':'No supported issues need action.'}</small></section><section className="clubOverviewQuickActions" aria-label="Quick actions"><button type="button" onClick={()=>setTab('classes')}>▦<span>Timetable</span></button><button type="button" onClick={()=>setTab('classes')}>▤<span>Class library</span></button><button type="button" onClick={()=>setTab('schedule')}>◷<span>Staffing</span></button><button type="button" onClick={()=>setTab('members')}>◉<span>Members</span></button></section></aside></div></div>;
  }
  function LegacyTodayCommandViewSchedule(){
    const summary=overviewSummary;const weekday=new Date(`${localDateKey()}T12:00:00`).getDay();const rows=(summary?.class_rows||[]).filter(row=>row.weekday===weekday).sort((a,b)=>a.start_time.localeCompare(b.start_time));
    if(!summary)return <div className="clubOverviewCompatibility" role="status">{overviewSummaryState==='loading'?'Loading today’s programme…':'Today’s club summary is not available yet.'}</div>;
    const now=new Date();const minutes=now.getHours()*60+now.getMinutes();const times=(time:string)=>{const [hour,minute]=time.slice(0,5).split(':').map(Number);return hour*60+minute};const completed=rows.filter(row=>times(row.finish_time)<=minutes).length;const current=rows.find(row=>times(row.start_time)<=minutes&&times(row.finish_time)>minutes);const next=rows.find(row=>times(row.start_time)>minutes);const unassigned=scheduledShifts.filter(shift=>shift.shift_date===localDateKey()&&!shift.profile_id&&shift.status!=='cancelled').length;const capacityRows=rows.filter(row=>row.capacity!==null).slice().sort((a,b)=>(a.available_spaces??999)-(b.available_spaces??999)).slice(0,3);
    return <div className="clubOverviewToday"><TodayMetricGrid summary={summary} rows={rows}/><div className="clubOverviewTodayLayout"><section className="clubOverviewProgramme clubOverviewTimelineCard"><div className="clubOverviewSectionHead"><div><span className="clubOverviewEyebrow">TODAY’S PROGRAMME</span><h2>Sessions in time order</h2></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>Open timetable</button></div><div className="clubOverviewTimeline">{rows.map(row=>{const used=row.capacity?Math.min(100,Math.max(0,row.active_places/row.capacity*100)):0;const status=row.available_spaces!==null&&row.available_spaces<0?'over':row.available_spaces===0?'full':row.capacity&&row.available_spaces!==null&&row.available_spaces/row.capacity<=.2?'near':row.active_places===0?'empty':'healthy';const phase=times(row.finish_time)<=minutes?'complete':current?.class_id===row.class_id?'current':next?.class_id===row.class_id?'next':'';return <article key={row.class_id} className={`clubOverviewTimelineRow ${status} ${phase}`}><time>{row.start_time.slice(0,5)}<small>{row.finish_time.slice(0,5)}</small></time><span className="clubOverviewTimelineMarker" aria-hidden="true"/><div><strong>{row.class_name}</strong><span>{row.venue_name}</span></div><div className="clubOverviewProgrammeCapacity"><small>{plural(row.active_places,'athlete')} · {row.capacity===null?'Capacity unavailable':`${row.active_places}/${row.capacity}`}</small><b>{row.available_spaces===null?'':`${row.available_spaces} ${row.available_spaces===1?'space':'spaces'} available`}</b>{row.waiting_list>0&&<em>{plural(row.waiting_list,'waiting')}</em>}{row.capacity!==null&&<span className="clubOverviewUtilisation"><b style={{width:`${used}%`}}/></span>}</div><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>View class</button></article>})}</div></section><aside className="clubOverviewTodayRail"><section className="clubOverviewRailCard clubOverviewProgress"><span className="clubOverviewEyebrow">DAY PROGRESS</span><strong>{rows.length?`${rows[0].start_time.slice(0,5)}–${rows[rows.length-1].finish_time.slice(0,5)}`:'No classes today'}</strong><small>{rows.length?`${completed} of ${rows.length} sessions complete · ${rows.length-completed-(current?1:0)} remaining`:'No sessions scheduled'}</small><span className="clubOverviewUtilisation"><b style={{width:rows.length?`${Math.round(completed/rows.length*100)}%`:'0%'}}/></span><b>{current?`${current.class_name} is happening now`:next?`Next: ${next.class_name} at ${next.start_time.slice(0,5)}`:rows.length?'Programme complete':'No classes today'}</b></section><section className="clubOverviewRailCard"><span className="clubOverviewEyebrow">CAPACITY WATCH</span>{capacityRows.length?capacityRows.map(row=><button type="button" key={row.class_id} onClick={()=>setTab('classes')}><strong>{row.class_name}</strong><small>{row.start_time.slice(0,5)} · {row.active_places}/{row.capacity} · {row.available_spaces} {row.available_spaces===1?'space':'spaces'}</small></button>):<small>All capacity information is unavailable.</small>}</section><section className="clubOverviewRailCard"><span className="clubOverviewEyebrow">NEEDS ATTENTION</span><strong>{unassigned?plural(unassigned,'unassigned staffing item'):'Everything is ready for today'}</strong><small>{unassigned?'Open staffing to assign support.':'No supported issues need action.'}</small></section><section className="clubOverviewQuickActions"><button type="button" onClick={()=>setTab('classes')}>▦<span>Timetable</span></button><button type="button" onClick={openClassLibrary}>▤<span>Class library</span></button><button type="button" onClick={()=>setTab('schedule')}>◷<span>Staffing</span></button><button type="button" onClick={()=>setTab('members')}>◉<span>Members</span></button></section></aside></div></div>;
  }
  function SessionSchedule({rows}:{rows:ClubOverviewSummary['class_rows']}){
    const now=new Date();const minutes=now.getHours()*60+now.getMinutes();const timeValue=(time:string)=>{const [h,m]=time.slice(0,5).split(':').map(Number);return h*60+m};
    return <div className="clubOverviewScheduleCards">{rows.map(row=>{const available=row.available_spaces;const status=available===null?'available':available<0?'over':available===0?'full':row.capacity&&available/row.capacity<=.2?'near':row.active_places===0?'empty':'available';const state=timeValue(row.finish_time)<=minutes?'completed':timeValue(row.start_time)<=minutes?'current':'upcoming';const availability=available===null?'Capacity unavailable':available<0?`${Math.abs(available)} over capacity`:available===0?'Full':`${available} available`;const used=row.capacity?Math.min(100,Math.max(0,row.active_places/row.capacity*100)):0;return <button key={row.class_id} type="button" className={`clubOverviewSessionCard ${status} ${state}`} aria-label={`Open ${row.class_name}, ${row.start_time.slice(0,5)} to ${row.finish_time.slice(0,5)}, ${availability}`} onClick={()=>setTab('classes')}><span className="clubOverviewSessionTime"><strong>{row.start_time.slice(0,5)}</strong><small>{row.finish_time.slice(0,5)}</small></span><span className="clubOverviewSessionDetails"><strong>{row.class_name}</strong><small>{row.venue_name||'Venue unavailable'}</small></span><span className="clubOverviewSessionCapacity"><b>{row.capacity===null?'Capacity unavailable':`${row.active_places} of ${row.capacity} places`}</b><em>{availability}</em>{row.waiting_list>0&&<small>{plural(row.waiting_list,'waiting')}</small>}{row.capacity!==null&&<i className="clubOverviewUtilisation"><b style={{width:`${used}%`}}/></i>}</span><span className="clubOverviewSessionChevron" aria-hidden="true">›</span></button>})}</div>;
  }
  function TodayQuickActions(){
    return <section className="clubOverviewQuickActions clubOverviewTodayQuickActions" aria-label="Quick actions"><button type="button" onClick={()=>setTab('classes')}>▦<span>Timetable</span></button><button type="button" onClick={openClassLibrary}>▤<span>Class library</span></button><button type="button" onClick={()=>setTab('schedule')}>◷<span>Staffing</span></button><button type="button" onClick={()=>setTab('members')}>◉<span>Members</span></button></section>;
  }
  function TodayOperationalRail({rows}:{rows:ClubOverviewSummary['class_rows']}){
    const now=new Date();const minutes=now.getHours()*60+now.getMinutes();const timeValue=(time:string)=>{const [hour,minute]=time.slice(0,5).split(':').map(Number);return hour*60+minute};
    const completed=rows.filter(row=>timeValue(row.finish_time)<=minutes).length;const current=rows.find(row=>timeValue(row.start_time)<=minutes&&timeValue(row.finish_time)>minutes);const next=rows.find(row=>timeValue(row.start_time)>minutes);
    const remaining=Math.max(0,rows.length-completed-(current?1:0));const progress=rows.length?Math.round(completed/rows.length*100):0;
    const capacityRows=rows.filter(row=>row.capacity!==null&&row.available_spaces!==null).slice().sort((a,b)=>{const aCritical=(a.available_spaces||0)<=0,bCritical=(b.available_spaces||0)<=0;if(aCritical!==bCritical)return aCritical?-1:1;if(aCritical)return (a.available_spaces||0)-(b.available_spaces||0);return (b.available_spaces||0)-(a.available_spaces||0)}).slice(0,3);
    const unassigned=scheduledShifts.filter(shift=>shift.shift_date===localDateKey()&&!shift.profile_id&&shift.status!=="cancelled").length;
    return <aside className="clubOverviewTodayRail"><section className="clubOverviewRailCard clubOverviewProgress"><span className="clubOverviewEyebrow">DAY PROGRESS</span><strong>{rows.length?`${rows[0].start_time.slice(0,5)}–${rows[rows.length-1].finish_time.slice(0,5)}`:'No classes today'}</strong><small>{rows.length?`${completed} of ${rows.length} sessions complete · ${remaining} remaining`:'No sessions scheduled'}</small><span className="clubOverviewUtilisation" role="progressbar" aria-label="Today’s programme progress" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100}><b style={{width:`${progress}%`}}/></span><b>{current?`${current.class_name} is happening now`:next?`Next: ${next.class_name} at ${next.start_time.slice(0,5)}`:rows.length?'Programme complete':'No classes today'}</b></section><section className="clubOverviewRailCard clubOverviewCapacityWatch"><span className="clubOverviewEyebrow">CAPACITY WATCH</span>{capacityRows.length?capacityRows.map(row=>{const available=row.available_spaces!;const availability=available<0?`${Math.abs(available)} over capacity`:available===0?'Full':`${available} available`;return <button type="button" key={row.class_id} onClick={()=>setTab('classes')}><strong>{row.class_name}</strong><small>{row.start_time.slice(0,5)} · {row.active_places}/{row.capacity} · {availability}{row.waiting_list>0?` · ${plural(row.waiting_list,'waiting')}`:''}</small></button>}):<small>All capacity information is unavailable.</small>}</section><section className="clubOverviewRailCard clubOverviewNeedsAttention"><span className="clubOverviewEyebrow">NEEDS ATTENTION</span><strong>{unassigned?plural(unassigned,'unassigned staffing item'):'Everything is ready for today'}</strong><small>{unassigned?'Open staffing to assign support.':'No supported issues need action.'}</small></section></aside>;
  }
  function TodayCommandView(){
    const summary=overviewSummary;const weekday=new Date(`${localDateKey()}T12:00:00`).getDay();const rows=(summary?.class_rows||[]).filter(row=>row.weekday===weekday).sort((a,b)=>a.start_time.localeCompare(b.start_time));
    if(!summary)return <div className="clubOverviewCompatibility" role="status">{overviewSummaryState==='loading'?'Loading today’s programme…':'Today’s club summary is not available yet.'}</div>;
    return <div className="clubOverviewToday"><TodayMetricGrid summary={summary} rows={rows}/><TodayQuickActions/><div className="clubOverviewTodayLayout"><section className="clubOverviewProgramme clubOverviewSchedulePanel" aria-labelledby="today-programme"><div className="clubOverviewSectionHead"><div><span className="clubOverviewEyebrow">TODAY’S PROGRAMME</span><h2 id="today-programme">Sessions in time order</h2></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>Open timetable</button></div>{rows.length?<SessionSchedule rows={rows}/>:<div className="clubOverviewAllClear"><strong>No classes today</strong><span>There are no published sessions for today.</span></div>}</section><TodayOperationalRail rows={rows}/></div></div>;
  }
  function TodayView(){
    const today=localDateKey();
    const classesToday=scheduledShifts.filter(s=>s.shift_date===today&&s.status!=="cancelled").sort((a,b)=>a.start_time.localeCompare(b.start_time));
    const unassigned=classesToday.filter(s=>!s.profile_id).length;
    return <div className="clubOverviewToday"><PageHead centered dashboard title={`Good ${new Date().getHours()<12?"morning":new Date().getHours()<18?"afternoon":"evening"}, ${initialProfile.full_name.split(" ")[0]}`} sub="Here’s what’s happening across your club today."><div className="clubOverviewViewSwitch" role="tablist" aria-label="Club Overview view"><button type="button" role="tab" aria-selected={overviewView==='today'} onClick={()=>setOverviewSection('today')}>Today</button><button type="button" role="tab" aria-selected={overviewView==='dashboard'} onClick={()=>setOverviewSection('dashboard')}>Dashboard</button></div></PageHead><p className="clubOverviewDate">{new Date(`${today}T12:00:00`).toLocaleDateString("en-GB",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</p><section className="clubOverviewSummaryGrid" aria-label="Today at a glance"><button className="clubOverviewSummaryCard" type="button" onClick={()=>setTab('schedule')}><strong>{classesToday.length}</strong><span>Classes today</span></button><div className="clubOverviewSummaryCard"><strong>—</strong><span>Athletes expected today</span><small>Unavailable from existing reads</small></div><div className="clubOverviewSummaryCard"><strong>—</strong><span>Registers completed / remaining</span><small>Unavailable from existing reads</small></div><button className="clubOverviewSummaryCard" type="button" onClick={()=>setTab('schedule')}><strong>{unassigned}</strong><span>Unassigned classes</span></button></section><section className="clubOverviewAttention" aria-labelledby="today-attention"><div><span className="clubOverviewEyebrow">PRIORITY</span><h2 id="today-attention">Needs attention</h2></div>{unassigned>0?<button className="clubOverviewAttentionItem" type="button" onClick={()=>setTab('schedule')}><strong>{unassigned} unassigned {unassigned===1?'class':'classes'}</strong><span>Open Schedule &amp; Staffing</span></button>:<div className="clubOverviewAllClear"><strong>All clear for today</strong><span>No supported staffing issues need attention.</span></div>}</section><section className="clubOverviewProgramme" aria-labelledby="today-programme"><div className="sectionHeader"><div><span className="clubOverviewEyebrow">TODAY</span><h2 id="today-programme">Today’s programme</h2></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('schedule')}>View staffing</button></div>{classesToday.length? <div className="clubOverviewTimeline">{classesToday.map(s=><article key={s.id} className="clubOverviewTimelineRow"><time>{s.start_time.slice(0,5)}<small>{s.finish_time.slice(0,5)}</small></time><div><strong>{s.class_name}</strong><span>{venueName(s.venue_id)}</span><small>{s.profile_id?"Assigned":"Unassigned"} · Register status unavailable</small></div><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>View class</button></article>)}</div>:<div className="clubOverviewAllClear"><strong>No classes scheduled today</strong><span>Published staffing data is currently available.</span></div>}</section><section className="clubOverviewQuickActions" aria-label="Quick actions"><button type="button" className="btn btnSecondary" onClick={()=>setTab('members')}>Add member</button><button type="button" className="btn btnSecondary" onClick={()=>setTab('members')}>Enrol athlete</button><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>Create class</button><button type="button" className="btn btnSecondary" onClick={()=>setTab('classes')}>Open today’s registers</button><button type="button" className="btn btnSecondary" onClick={()=>setTab('schedule')}>Add shift</button></section></div>;
  }
  function DashboardView(){
    if(isAdmin&&overviewHydrated)return <div className="clubOverviewShell" ref={overviewContentRef}><ClubOverviewHero/>{overviewView==='dashboard'&&<DashboardPanelTabs/>}<DashboardContent/></div>;
    return DashboardContent();
  }
  function DashboardPanelTabs(){
    const panels:[typeof dashboardPanel,string][]=[['overview','Overview'],['members','Members'],['classes','Classes & Capacity'],['staffing','Staffing & Costs'],['finance','Finance']];
    const move=(event:React.KeyboardEvent<HTMLDivElement>)=>{const index=panels.findIndex(([id])=>id===dashboardPanel);const next=event.key==='Home'?0:event.key==='End'?panels.length-1:event.key==='ArrowRight'?(index+1)%panels.length:event.key==='ArrowLeft'?(index-1+panels.length)%panels.length:-1;if(next>=0){event.preventDefault();setDashboardOverviewPanel(panels[next][0]);}};
    return <div className="clubOverviewPanelTabs" role="tablist" aria-label="Dashboard sections" onKeyDown={move}>{panels.map(([id,label])=><button key={id} type="button" role="tab" aria-selected={dashboardPanel===id} aria-controls={`club-overview-${id}`} onClick={()=>setDashboardOverviewPanel(id)}>{label}</button>)}</div>;
  }
  function renderClassCapacityGroups({rows}:{rows:ClubOverviewSummary['class_rows']}){
    const groups=sortCapacityGroups(groupCapacityRows(rows),capacitySort);
    const closeDetail=()=>{setCapacityDetail(null);requestAnimationFrame(()=>capacityDetailReturnFocus.current?.focus())};
    return <section className="clubOverviewPanel" role="tabpanel" id="club-overview-classes" aria-label="Classes and capacity"><div className="clubOverviewPanelIntro"><div><h2>Classes &amp; Capacity</h2><p>Totals are across weekly recurrences, not unique athletes.</p></div><label className="clubOverviewSort">Sort <select value={capacitySort} onChange={event=>setCapacitySort(event.target.value as typeof capacitySort)}><option value="priority">Operational priority</option><option value="name">Class name</option><option value="spaces">Most spaces</option></select></label></div><div className="clubOverviewCapacityGroups">{groups.map(group=><article key={group.profileId} className={`clubOverviewCapacityGroup ${group.state}`}><div className="clubOverviewCapacityGroupHead"><button type="button" ref={capacityDetail?.profileId===group.profileId?capacityDetailReturnFocus:undefined} aria-haspopup="dialog" onClick={event=>{capacityDetailReturnFocus.current=event.currentTarget;setCapacityDetail(group)}}><span><strong>{group.name}</strong><small>{plural(group.rows.length,'weekly session')} · {plural(group.active,'session place')} occupied</small></span><b>{group.capacity===null?'Capacity unavailable':`${group.active} / ${group.capacity} · ${group.available} ${group.available===1?'space':'spaces'} · ${plural(group.waiting,'waiting')}`}</b></button><button className="btn btnSecondary" type="button" onClick={()=>setTab('classes')}>Open class</button></div>{group.capacity!==null&&<span className="clubOverviewUtilisation" role="progressbar" aria-label={`${group.name} utilisation`} aria-valuenow={group.utilisation} aria-valuemin={0} aria-valuemax={100}><b style={{width:`${Math.min(100,group.utilisation)}%`}}/></span>}</article>)}</div>{capacityDetail&&<div className="clubOverviewCapacityDrawerBackdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)closeDetail()}}><aside className="clubOverviewCapacityDrawer" role="dialog" aria-modal="true" aria-labelledby="capacity-detail-title" onKeyDown={event=>{if(event.key==='Escape'){event.preventDefault();closeDetail();return}if(event.key==='Tab'){const focusable=[...event.currentTarget.querySelectorAll<HTMLElement>('button,[href],[tabindex]:not([tabindex="-1"])')];const first=focusable[0],last=focusable.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last?.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first?.focus()}}}}><header><div><span className="clubOverviewEyebrow">CLASS RECURRENCES</span><h2 id="capacity-detail-title">{capacityDetail.name}</h2></div><button type="button" className="iconButton" autoFocus aria-label="Close class capacity details" onClick={closeDetail}>×</button></header><div>{capacityDetail.rows.map(row=><article key={row.class_id}><strong>{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][row.weekday]} · {row.start_time.slice(0,5)}–{row.finish_time.slice(0,5)}</strong><span>{row.venue_name}</span><small>{plural(row.active_places,'active place')} · {plural(row.waiting_list,'waiting')} · {row.capacity===null?'Capacity unavailable':`${row.capacity} capacity · ${row.available_spaces} available`}</small>{row.capacity!==null&&<span className="clubOverviewUtilisation" role="progressbar" aria-label={`${row.class_name} utilisation`} aria-valuenow={Math.round(row.active_places/row.capacity*100)} aria-valuemin={0} aria-valuemax={100}><b style={{width:`${Math.min(100,Math.round(row.active_places/row.capacity*100))}%`}}/></span>}</article>)}</div></aside></div>}</section>;
  }
  function renderOverviewPanel({summary,cards}:{summary:ClubOverviewSummary|null;cards:readonly (readonly [string,number|undefined])[]}){
    const snapshot=summary?.snapshot;const capacityGroups=groupCapacityRows(summary?.class_rows||[]);const constrained=capacityGroups.filter(group=>group.state==='over'||group.state==='full'||group.state==='nearly-full').length;const capacityRows=summary?.class_rows||[];const workforce=staff.filter(person=>person.is_active).length;const outstanding=Math.max(0,adminRows.length-submittedCount);const overCapacity=capacityRows.filter(row=>(row.available_spaces??0)<0).length;const fullWithWaiting=capacityRows.filter(row=>row.available_spaces===0&&row.waiting_list>0).length;const placesToFill=capacityRows.filter(row=>row.capacity!==null&&row.available_spaces!==null&&row.available_spaces>0).sort((a,b)=>(b.available_spaces||0)-(a.available_spaces||0)||a.class_name.localeCompare(b.class_name)).slice(0,3);const kpis=[{label:'Active families',value:snapshot?.active_family_accounts,detail:snapshot?`${plural(snapshot.active_athletes,'active athlete')} across your club`:undefined,icon:<UsersIcon/>},{label:'Active athletes',value:snapshot?.active_athletes,detail:snapshot?plural(snapshot.active_enrolments,'active enrolment'):undefined,icon:<UserIcon/>},{label:'Active enrolments',value:snapshot?.active_enrolments,detail:snapshot?plural(snapshot.waiting_enrolments,'waiting-list enrolment'):undefined,icon:<CheckIcon/>},{label:'Trials',value:snapshot?.trial_enrolments,detail:snapshot?plural(snapshot.paused_enrolments,'paused enrolment'):undefined,icon:<CalendarIcon/>}];const attention=[unassignedScheduleCount>0?{severity:'urgent',count:unassignedScheduleCount,text:plural(unassignedScheduleCount,'staffing assignment needed'),panel:'staffing' as const}:null,outstanding>0?{severity:'warning',count:outstanding,text:plural(outstanding,'timesheet awaiting submission'),panel:'staffing' as const}:null,overCapacity>0?{severity:'urgent',count:overCapacity,text:plural(overCapacity,'session over capacity'),panel:'classes' as const}:null,fullWithWaiting>0?{severity:'warning',count:fullWithWaiting,text:plural(fullWithWaiting,'full session with a waiting list'),panel:'classes' as const}:null,snapshot&&snapshot.waiting_enrolments>0?{severity:'notice',count:snapshot.waiting_enrolments,text:plural(snapshot.waiting_enrolments,'waiting-list enrolment requiring review'),panel:'members' as const}:null].filter(Boolean) as {severity:string;count:number;text:string;panel:'members'|'classes'|'staffing'}[];
    return <div className="clubOverviewOverview" id="club-overview-overview" role="tabpanel" aria-label="Overview"><section className="clubOverviewKpis" aria-label="Club snapshot">{kpis.map(({label,value,detail,icon})=><article key={label}><span className="clubOverviewKpiIcon" aria-hidden="true">{icon}</span><strong>{typeof value==='number'?value.toLocaleString('en-GB'):'Unavailable'}</strong><span>{label}</span>{detail&&<small>{detail}</small>}</article>)}</section><section className="clubOverviewActionCentre" aria-labelledby="action-centre-title"><div className="clubOverviewActionHeading"><span className="clubOverviewEyebrow">OPERATIONS</span><h2 id="action-centre-title">Action centre</h2></div><div className="clubOverviewActionGrid"><article className="clubOverviewAttentionList"><h3>Needs attention</h3>{attention.length?attention.map(item=><button type="button" key={`${item.panel}-${item.text}`} className={`clubOverviewAttentionRow ${item.severity}`} onClick={()=>setDashboardOverviewPanel(item.panel)}><span aria-hidden="true">{item.severity==='urgent'?<span>!</span>:<ClockIcon/>}</span><div><strong>{item.count}</strong><small>{item.text}</small></div><b>View {item.panel==='classes'?'capacity':item.panel}</b></button>):<div className="clubOverviewAllClear"><CheckIcon/><div><strong>Everything looks on track</strong><span>No supported issues need action.</span></div></div>}</article><article className="clubOverviewPlaces"><h3>Places to fill</h3>{placesToFill.length?<div>{placesToFill.map(row=>{const utilisation=Math.round(row.active_places/(row.capacity||1)*100);return <button type="button" key={row.class_id} onClick={()=>setDashboardOverviewPanel('classes')} aria-label={`Open capacity for ${row.class_name}`}><div><strong>{row.class_name}</strong><small>{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][row.weekday]} · {row.start_time.slice(0,5)} · {row.venue_name}</small></div><b>{row.active_places}/{row.capacity} · {row.available_spaces} available</b><span className="clubOverviewUtilisation" role="progressbar" aria-label={`${row.class_name} utilisation`} aria-valuenow={utilisation} aria-valuemin={0} aria-valuemax={100}><i style={{width:`${Math.min(100,utilisation)}%`}}/></span></button>})}</div>:<div className="clubOverviewAllClear"><CheckIcon/><div><strong>All current sessions are full</strong><span>There are no available places to highlight.</span></div></div>}</article></div></section><section className="clubOverviewExplore" aria-labelledby="explore-club"><div><span className="clubOverviewEyebrow">NAVIGATION</span><h2 id="explore-club">Explore your club</h2></div><div>{[{panel:'members' as const,title:'Members & Enrolments',headline:snapshot?plural(snapshot.waiting_enrolments,'waiting-list enrolment'):'Member data unavailable',detail:snapshot?'Review member and enrolment activity.':'',icon:<UsersIcon/>},{panel:'classes' as const,title:'Classes & Capacity',headline:`${constrained} full or nearly full`,detail:snapshot?`${plural(snapshot.active_weekly_sessions,'weekly session')} scheduled.`:'',icon:<CalendarIcon/>},{panel:'staffing' as const,title:'Staffing & Costs',headline:plural(unassignedScheduleCount,'unassigned staffing item'),detail:outstanding?plural(outstanding,'timesheet awaiting submission'):'Staffing is up to date.',icon:<ClockIcon/>}].map(card=><button type="button" key={card.panel} className="clubOverviewExploreCard" onClick={()=>setDashboardOverviewPanel(card.panel)} aria-label={`Open ${card.title}`}><span aria-hidden="true">{card.icon}</span><div><strong>{card.title}</strong><b>{card.headline}</b><small>{card.detail}</small></div><i aria-hidden="true">→</i></button>)}</div></section><div className="clubOverviewFinancePlanned"><PoundIcon/><strong>Member Finance</strong><span>Member billing is not connected yet</span><b>Planned</b></div></div>;
  }
  function DashboardContent(){
    if(isAdmin&&!overviewHydrated)return <div className="clubOverviewToday"><PageHead centered dashboard title="Club Overview" sub="Your club at a glance."><div className="clubOverviewViewSwitch" role="tablist" aria-label="Club Overview view"><button type="button" role="tab" aria-selected={false}>Today</button><button type="button" role="tab" aria-selected={false}>Dashboard</button></div></PageHead><div className="clubOverviewHydrationSkeleton" role="status" aria-label="Loading Club Overview" /></div>;
    if(isAdmin&&overviewView==='today')return TodayCommandView();
    if(isAdmin){
      const summary=overviewSummary;
      const value=(n:number|undefined)=>overviewSummaryState==='loaded'&&typeof n==='number'?n.toLocaleString('en-GB'):'Unavailable';
      const cards=[['Active family accounts',summary?.snapshot.active_family_accounts],['Active athletes',summary?.snapshot.active_athletes],['Active enrolments',summary?.snapshot.active_enrolments],['Active trials',summary?.snapshot.trial_enrolments]] as const;
      if((dashboardPanel as string)==='overview')return renderOverviewPanel({summary,cards});
      if((dashboardPanel as string)==='classes')return renderClassCapacityGroups({rows:summary?.class_rows||[]});
      if(dashboardPanel==='staffing')return <section className="clubOverviewPanel" role="tabpanel" aria-label="Staffing and costs"><div className="clubOverviewPanelIntro"><h2>Staffing &amp; Costs</h2><p>Current staffing health, time and cost position.</p></div><div className="clubOverviewStaffingControls"><MonthNavigation/><button className="btn btnSecondary" type="button" onClick={()=>{setAdminPersonalRota(true);setTab('schedule')}}>My Schedule</button></div>{DashboardLegacyView(true)}</section>;
      if(dashboardPanel==='finance')return <section className="clubOverviewPanel clubOverviewCompatibility" role="tabpanel" aria-label="Finance"><h2>Member billing is not connected yet.</h2><p>Staff costs, invoices and forecasting remain available under Staffing &amp; Costs.</p></section>;
      if(dashboardPanel==='members')return <section className="clubOverviewPanel" role="tabpanel" aria-label="Members"><div className="clubOverviewPanelIntro"><h2>Members &amp; Enrolments</h2><button className="btn btnSecondary" type="button" onClick={()=>setTab('members')}>Open Members</button></div><div className="clubOverviewSnapshotGrid">{cards.map(([label,n])=><div key={label}><strong>{value(n)}</strong><span>{label}</span></div>)}</div>{summary&&<div className="clubOverviewStatusBar" aria-label="Enrolment composition"><span>Active {summary.snapshot.active_enrolments}</span><span>Trial {summary.snapshot.trial_enrolments}</span><span>Waiting {summary.snapshot.waiting_enrolments}</span><span>Paused {summary.snapshot.paused_enrolments}</span></div>}</section>;
      return renderOverviewPanel({summary,cards});
    }
    return DashboardLegacyView();
  }
  function DashboardLegacyView(embedded=false){
    ...
    if(isAdmin)return <>{!embedded&&<PageHead centered dashboard title={`Good ${new Date().getHours()<12?"morning":new Date().getHours()<18?"afternoon":"evening"}, ${initialProfile.full_name.split(" ")[0]}`} sub="Your current staffing, timesheet and invoice position."><div className="v434OverviewHeadControls"><div className="clubOverviewViewSwitch" role="tablist" aria-label="Club Overview view"><button type="button" role="tab" aria-selected={overviewView==='today'} onClick={()=>setOverviewSection('today')}>Today</button><button type="button" role="tab" aria-selected={overviewView==='dashboard'} onClick={()=>setOverviewSection('dashboard')}>Dashboard</button></div><MonthNavigation/><button className="btn btnSecondary" onClick={()=>{setAdminPersonalRota(true);setTab("schedule")}}>My Schedule</button></div></PageHead>}
      ...
      <div className="grid grid4 section forecastCards"><StatCard label="Normal staffing cost" value={money(staffingCosts.normalStaffingCost)} foot="Based on regular classes" icon={<CalendarIcon/>}/><StatCard label="Current forecast" value={money(staffingCosts.currentForecast)} foot={`${unassignedScheduleCount} unassigned shifts`} icon={<PoundIcon/>}/><StatCard label="Actual cost so far" value={actualCostDisplay} foot={actualCostFoot} icon={<CheckIcon/>}/><StatCard label="Forecast variance" value={money(staffingCosts.forecastVariance)} foot={staffingCosts.forecastVariance>0?"Above normal plan":"At / below normal plan"} icon={<ChartIcon/>}/></div>
  ...
  function WorkforceView(){
    ...
    staff.forEach(person=>{
      ...
      organisationIds.forEach(organisationId=>{
        ...
        const volunteerHours=personShifts.filter(shift=>shift.payment_type==="volunteer").reduce((total,shift)=>total+shiftHours(shift),0);
        const salariedHours=employmentType==="salaried"?hours-volunteerHours:0;
    ...
    const approvedExpenses=expenses.filter(expense=>expense.status==="approved"&&expense.expense_date>=from&&expense.expense_date<=to);
    const approvedExpenseTotal=approvedExpenses.reduce((total,expense)=>total+Number(expense.amount),0);
    const totalWorkforceCost=salaryTotal+variableStaffing+approvedExpenseTotal;
    const labels:Record<WorkforceRow["employmentType"],string>={hourly:"Hourly",salaried:"Salaried",volunteer:"Volunteer"};
    const breakdown=(type:"hourly"|"salaried")=>{const typeRows=rows.filter(row=>row.employmentType===type);return{staff:new Set(typeRows.map(row=>row.person.id)).size,hours:typeRows.reduce((total,row)=>total+(type==="hourly"?row.standardHours+row.enhancedHours:row.hours),0),volunteerHours:typeRows.reduce((total,row)=>total+row.volunteerHours,0),cost:typeRows.reduce((total,row)=>total+row.cost,0)}};
    const hourlyRows=visibleRows.filter(row=>row.employmentType==="hourly"),salariedRows=visibleRows.filter(row=>row.employmentType==="salaried");
    ...
    return <div className="v12WorkforcePage">
      ...
      <div className="v12WorkforceSummary">
        <StatCard label="Salaried" value={money(salaryTotal)} foot={monthLabel(month)} icon={<PoundIcon/>}/><StatCard label="Hourly Staffing" value={money(variableStaffing)} foot="Paid work cost" icon={<ClockIcon/>}/><StatCard label="Volunteer Work" value={`${volunteerHours.toFixed(2)}h`} foot="Unpaid hours · £0 cost" icon={<UsersIcon/>}/>
        <details className="card statCard v14WorkforceExpenses"><summary><span className="statIcon"><InvoiceIcon/></span><span className="statLabel">Approved Expenses</span><strong className="statValue">{money(approvedExpenseTotal)}</strong><span className="statFoot">Awaiting reimbursement · {approvedExpenses.length} approved {approvedExpenses.length===1?"claim":"claims"}</span></summary><div className="v14WorkforceExpenseList">{approvedExpenses.map(expense=><article key={expense.id}><strong>{expense.claimant?.full_name||profileById(expense.profile_id)?.full_name||"Staff"}</strong><time>{dateText(expense.expense_date)}</time><span>{expenseCategoryLabel(expense.category)}</span><b>{money(Number(expense.amount))}</b></article>)}{!approvedExpenses.length&&<p>No approved expenses awaiting reimbursement.</p>}<div className="v14WorkforceExpenseFoot"><span>Total</span><b>{money(approvedExpenseTotal)}</b></div><button className="btn btnSecondary" type="button" onClick={()=>setTab("expenses")}>View Expenses</button></div></details>
      </div>
      <section className="card v14WorkforceTotal"><div><span>Total Workforce Cost</span><strong>{money(totalWorkforceCost)}</strong><small>Management cost view · {monthLabel(month)}</small></div><dl><div><dt>Salary</dt><dd>{money(salaryTotal)}</dd></div><div><dt>Hourly staffing</dt><dd>{money(variableStaffing)}</dd></div><div><dt>Expenses</dt><dd>{money(approvedExpenseTotal)}</dd></div><div><dt>Total Workforce Cost</dt><dd>{money(totalWorkforceCost)}</dd></div></dl></section>
      <section className="card v12WorkforceBreakdown"><div className="sectionHeader"><div><h2>Workforce breakdown</h2><p>Headcount, worked hours and employment cost by employment type.</p></div></div><div className="v12EmploymentBreakdown">{(["salaried","hourly"] as const).map(type=>{const value=breakdown(type);return <article key={type}><span>{type==="salaried"?"Salaried Staff":"Hourly Staff"}</span><strong>{value.staff} Staff</strong><div><small>{type==="hourly"?"Paid hours":"Worked hours"}</small><b>{value.hours.toFixed(2)}h</b>{type==="hourly"&&<><small>Volunteer hours</small><b>{value.volunteerHours.toFixed(2)}h</b></>}<small>{type==="salaried"?"Monthly cost":"Cost"}</small><b>{money(value.cost)}</b></div></article>})}</div></section>
      <section className="v12WorkforceDetails">
        ...
        <DetailTable title="Hourly staff" items={hourlyRows} columns={["Employee","Paid hours","Volunteer hours","Estimated cost","Standard rate","Enhanced rate"]} render={row=><tr key={row.key}><td>{row.person.full_name}</td><td>{(row.standardHours+row.enhancedHours).toFixed(2)}h</td><td>{row.volunteerHours.toFixed(2)}h</td><td>{money(row.cost)}</td><td>{money(row.standardRate)}</td><td>{money(row.enhancedRate)}</td></tr>}/>
      </section>
      <section className="card v12WorkforceTable"><div className="sectionHeader"><div><h2>Staff</h2><p>Select a row to open the existing Staff Profile.</p><small className="v14WorkforceHelp">Employment is the person’s arrangement. Standard, Enhanced, Volunteer and Salary Hours are work types for the selected month.</small></div><div className="searchBar"><SearchIcon/><input value={workforceSearch} onChange={event=>setWorkforceSearch(event.target.value)} placeholder="Search workforce…"/></div></div><div className="tableWrap"><table><thead><tr><th>Staff</th><th>Employment</th><th>Workforce Cost</th><th title="Standard-rate work in the selected month">Standard</th><th title="Enhanced-rate work in the selected month">Enhanced</th><th title="Unpaid volunteer work in the selected month">Volunteer</th><th title="Worked hours covered by salary">Salary Hours</th><th>Total Hours</th><th>Status</th></tr></thead><tbody>{visibleRows.map(row=><tr key={row.key} onClick={()=>openStaffEdit(row.person)} className="v12WorkforceRow"><td><button type="button">{row.person.full_name}</button></td><td>{labels[row.employmentType]}</td><td>{money(row.cost)}</td><td>{row.standardHours.toFixed(2)}h</td><td>{row.enhancedHours.toFixed(2)}h</td><td>{row.volunteerHours.toFixed(2)}h</td><td>{row.salariedHours.toFixed(2)}h</td><td>{row.hours.toFixed(2)}h</td><td><StatusPill status={row.person.is_active?"active":"inactive"}/></td></tr>)}</tbody></table></div>{!visibleRows.length&&<div className="empty">No Data</div>}</section>
  ...
  function IntelligentStaffingDrawer(){
    ...
    const assign=(coachId:string,trigger:HTMLButtonElement)=>{const profile=profileById(coachId);if(!profile)return;paymentAssignmentReturnFocus.current=trigger;const employmentType=employmentTypeForProfile(profile,shift.shift_date);setPaymentAssignment({shift,profile,paymentType:employmentType==="volunteer"?"volunteer":"standard"})};
    return <div className="v11DrawerBackdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget){setStaffingRecommendationShift(null);setStaffingQualificationContext(null)}}}><aside className="v11StaffingDrawer" role="dialog" aria-modal="true" aria-labelledby="staffing-drawer-title" aria-hidden={paymentAssignment?true:undefined} inert={paymentAssignment?true:undefined}>
      ...
      <div className="v11DrawerBody">{LinkedClassAction({shift})}<div className="v420CoachSearch"><span aria-hidden="true">⌕</span><input type="search" value={coachAssignmentSearch} onChange={event=>setCoachAssignmentSearch(event.target.value)} placeholder="Search coaches..." aria-label="Search coaches"/></div>
        ...
        {coachRows.map((row,index)=><div className="v11RecommendationWrap" key={row.coach.id}>{index===1&&<h3 className="v11OtherCoaches">Other Coaches</h3>}<article className={`v11RecommendationCard ${index===0?"best":""} ${row.state.state!=="available"?"warning":""} ${shift.profile_id===row.coach.id?"selected":""}`}>{index===0&&<div className="v11BestLabel">Best recommendation</div>}<div className="v11RecommendationHead"><div className="v11CoachAvatar">{initials(row.coach.full_name)}</div><div><h3>{row.coach.full_name}</h3><span>{shift.profile_id===row.coach.id?"Currently assigned · ":""}{row.matchLabel}</span>{shift.profile_id===row.coach.id&&paymentLabelForShift(shift)&&<span className={`v13PaymentBadge ${(paymentLabelForShift(shift)||"").toLowerCase()}`}>{paymentLabelForShift(shift)}</span>}</div><div><small>Estimated cost</small><strong>£{(shift.profile_id===row.coach.id?scheduleHours(shift)*paymentRateFor(row.coach,shift.shift_date,shift.payment_type):row.result.estimatedStaffingCost).toFixed(2)}</strong></div></div>
          ...
          <button className={`btn ${row.state.state==="available"?"btnPrimary":"btnAccent"}`} type="button" disabled={shift.status==="cancelled"||shift.status==="confirmed"} onClick={event=>assign(row.coach.id,event.currentTarget)}>{row.state.state==="available"?`Assign ${staffingRole==="lead"?"Lead":"Assistant"}`:"Assign Anyway"}</button>
        </article></div>)}
  ...
  function closePaymentAssignment(){
    setPaymentAssignment(null);
    window.requestAnimationFrame(()=>paymentAssignmentReturnFocus.current?.focus());
  }
  ...
  function PaymentAssignmentModal(){
    ...
    const confirmAssignment=async()=>{
      ...
      if(changed){setStaffingRecommendationShift(current=>current?.id===assignment.shift.id?{...current,profile_id:assignment.profile.id,payment_type:paymentType}:current);closePaymentAssignment()}
    };
    return <div className="modalBackdrop v13PaymentAssignmentLayer" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)closePaymentAssignment()}}><div className="modal v13PaymentAssignmentModal" role="dialog" aria-modal="true" aria-labelledby="payment-assignment-title"><div className="modalHead"><div><h2 id="payment-assignment-title">Confirm assignment</h2><p className="muted">{assignment.profile.full_name} · {assignment.shift.class_name}</p></div><button className="iconButton" type="button" aria-label="Close payment type selector" autoFocus onClick={closePaymentAssignment}>×</button></div><div className="modalBody">
      {LinkedClassAction({shift:assignment.shift})}<PaymentTypeControl profile={assignment.profile} date={assignment.shift.shift_date} value={assignment.paymentType} name="assignment-payment-type" onChange={paymentType=>setPaymentAssignment({...assignment,paymentType})}/>
    </div><div className="modalFoot"><button className="btn btnSecondary" type="button" onClick={closePaymentAssignment}>Cancel</button><button className="btn btnPrimary" type="button" disabled={saving} onClick={()=>void confirmAssignment()}>{saving?"Assigning…":"Confirm Assignment"}</button></div></div></div>;
  }
  ...
  function LinkedClassAction({shift}:{shift:ScheduledShift}){
    const linked=classes.find(c=>c.id===shift.class_id&&c.class_profile_id);
    return isAdmin&&linked?<button className="btn btnSecondary" type="button" onClick={e=>openEditClass(linked,e.currentTarget)}>Edit class</button>:null;
  }
  function AdminScheduleShiftModal(){
    ...
    return <div className="modalBackdrop"><div className="modal v311AdminShiftModal v405ScheduleControlModal">
      ...
      <div className="modalFoot v405ScheduleControlFoot"><div className="v311AdminActions">
        ...
        {s.status!=="cancelled"&&<button className="btn btnSecondary" onClick={()=>{setAdminScheduleShift(null);void openAdjustment(s)}}>{s.status==="confirmed"?"Correct actual time":"Edit actual time"}</button>}
      </div></div>
    </div></div>
  }
  ...
  function ConfirmShiftModal(){
    ...
    return <div className="modalBackdrop"><div className="modal v31ConfirmModal">
      ...
      <div className="modalBody v31ConfirmBody">
        ...
        {LinkedClassAction({shift:s})}<div className="v31ConfirmPrompt"><strong>Did you work this session as planned?</strong><span>If the time changed, adjust it before confirming.</span></div>
      </div>
  ...
  function DailyConfirmationModal(){
    ...
    const eligible=eligibleDailyConfirmations(review.date,review.profileId).filter(shift=>Boolean(review.actuals[shift.id])).sort((a,b)=>a.start_time.localeCompare(b.start_time));
    ...
    const actualHours=(shift:ScheduledShift)=>validateActualTimes(review.actuals[shift.id])?0:scheduleHours({...shift,start_time:review.actuals[shift.id].start,finish_time:review.actuals[shift.id].finish,break_minutes:review.actuals[shift.id].breakMinutes});
    const expectedPay=(shift:ScheduledShift)=>employmentTypeFor(shift)==="hourly"?actualHours(shift)*rateFor(shift):0;
    const totalHours=selected.reduce((total,shift)=>total+actualHours(shift),0);
  ...
  function ShiftModal(){
    return <div className="modalBackdrop"><div className="modal"><div className="modalHead"><h2>{!shiftEditable?"Shift details":shiftModal?.id?(shiftModal.approval_status==="pending"?"Review extra shift":"Edit shift"):"Add extra shift"}</h2><button className="iconButton" onClick={()=>setShiftModal(null)}>×</button></div><fieldset className="modalBody" disabled={!shiftEditable}><div className="field"><label>Date</label><input disabled={Boolean(shiftModal?.id)} type="date" value={shiftModal!.shift_date} onChange={e=>setShiftModal({...shiftModal!,shift_date:e.target.value})}/></div><div className="grid grid2"><div className="field"><label>Start</label><input type="time" value={shiftModal!.start_time.slice(0,5)} onChange={e=>setShiftModal({...shiftModal!,start_time:e.target.value})}/></div><div className="field"><label>Finish</label><input type="time" value={shiftModal!.finish_time.slice(0,5)} onChange={e=>setShiftModal({...shiftModal!,finish_time:e.target.value})}/></div></div><div className="field"><label>Break (minutes)</label><input type="number" min={0} value={shiftModal!.break_minutes} onChange={e=>setShiftModal({...shiftModal!,break_minutes:Number(e.target.value)})}/></div><div className="field"><label>Session / group</label><input disabled={Boolean(shiftModal?.scheduled_shift_id)||shiftModal?.approval_status==="pending"} value={shiftModal!.session_location||""} onChange={e=>setShiftModal({...shiftModal!,session_location:e.target.value})} placeholder="e.g. competition, camp, meeting, cover"/></div><div className="field"><label>Notes</label><textarea disabled={Boolean(shiftModal?.scheduled_shift_id)||shiftModal?.approval_status==="pending"} value={shiftModal!.notes||""} onChange={e=>setShiftModal({...shiftModal!,notes:e.target.value})}/></div></fieldset><div className="modalFoot"><div>{shiftEditable&&shiftModal?.id&&!shiftModal.scheduled_shift_id&&<button className="btn btnDanger" onClick={deleteShift}>Delete shift</button>}</div><div className="row">{shiftEditable&&isAdmin&&shiftModal?.id&&shiftModal.approval_status==="pending"&&<><button className="btn btnDanger" onClick={()=>rejectExtraShift(shiftModal)}>Reject</button><button className="btn btnAccent" onClick={()=>approveExtraShift(shiftModal)}>Approve</button></>}<button className="btn btnSecondary" onClick={()=>setShiftModal(null)}>Cancel</button>{shiftEditable&&(!isAdmin||shiftModal?.approval_status!=="pending")&&<button className="btn btnPrimary" onClick={saveShift}>{isAdmin?"Save shift":"Send for approval"}</button>}</div></div></div></div>
  }
  ...
  function InviteModal(){
    return <div className="modalBackdrop"><form className="modal v323CreateAccount" onSubmit={sendInvite}><div className="modalHead"><div><h2>Create staff member</h2><p className="muted" style={{margin:"4px 0 0",fontSize:11}}>Save their profile, with optional portal access.</p></div><button type="button" className="iconButton" onClick={()=>setInviteOpen(false)}>×</button></div><div className="modalBody">
      ...
      <div className="grid grid2"><div className="field"><label>Account type</label><select value={inviteRole} onChange={e=>setInviteRole(e.target.value as any)} disabled={!canManageRoles}><option value="coach">Coach</option>{canManageRoles&&<option value="org_admin">Club administrator</option>}</select></div><div className="field"><label>Employment</label><select value={invite.employmentType} onChange={e=>setInvite({...invite,employmentType:e.target.value as "hourly"|"salaried"})}><option value="hourly">Hourly</option><option value="salaried">Salaried</option></select></div></div>
      {invite.employmentType==="hourly"?<><div className="grid grid2"><div className="field"><label>Standard Rate</label><input type="number" min={0} step="0.01" value={invite.rate} onChange={e=>setInvite({...invite,rate:e.target.value})} required/></div><div className="field"><label>Enhanced Rate</label><input type="number" min={0} step="0.01" value={invite.enhancedRate} onChange={e=>setInvite({...invite,enhancedRate:e.target.value})} placeholder={invite.rate||"0.00"}/></div></div><label className="v321ForceCheck"><input type="checkbox" checked={invite.canVolunteer} onChange={e=>setInvite({...invite,canVolunteer:e.target.checked})}/><span><strong>Can Volunteer</strong><small>Allow volunteer payment type when assigning work.</small></span></label></>:<div className="grid grid3"><div className="field"><label>Annual Salary</label><input type="number" min={0} step="0.01" value={invite.annualSalary} onChange={e=>setInvite({...invite,annualSalary:e.target.value})} required/></div><div className="field"><label>Contracted Weekly Hours</label><input type="number" min={0} max={168} step="0.25" value={invite.contractedWeeklyHours} onChange={e=>setInvite({...invite,contractedWeeklyHours:e.target.value})} required/></div><div className="field"><label>Working Weeks Per Year</label><input type="number" min={1} max={52} step="0.5" value={invite.workingWeeksPerYear} onChange={e=>setInvite({...invite,workingWeeksPerYear:e.target.value})} required/></div></div>}
